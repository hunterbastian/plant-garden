...
interface SeedBagProps {
  onShake: (bagX: number, bagY: number) => void
...
export function SeedBag({ onShake, containerRef }: SeedBagProps) {
  ...
  const dropSeeds = useCallback((bagX: number, bagY: number) => {
    setShowSeeds(true)
    onShake(bagX, bagY)
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    // Tilt based on horizontal movement delta
    ...
    const avgVel = velocityRef.current.reduce((a, b) => a + b, 0) / (velocityRef.current.length || 1)
    const now = Date.now()
    ...
    if (dirCountRef.current >= 3 && avgVel > 3 && now - lastShakeRef.current > 600) {
      ...
      // Calculate bag position relative to container
      if (bagRef.current && containerRef.current) {
        const bagRect = bagRef.current.getBoundingClientRect()
        const containerRect = containerRef.current.getBoundingClientRect()
        const relX = bagRect.left + bagRect.width / 2 - containerRect.left
        const relY = bagRect.bottom - containerRect.top
        dropSeeds(relX, relY)
      }
    }
    ...
    >
      ...
      {showSeeds && (
        <div className="absolute left-1/2 -translate-x-1/2" style={{ bottom: -4 }}>
          ...
          <div className="animate-seed-drop-2 absolute rounded-full" style={{ height: 4, width: 4, backgroundColor: "#5a5347", marginLeft: 8 }} />
          <div className="animate-seed-drop-3 absolute rounded-full" style={{ height: 5, width: 5, backgroundColor: "#3d3832", marginLeft: -8 }} />
        </div>
      )}
      ...
      {!hasInteracted && (
        <p
          className="absolute left-1/2 -translate-x-1/2 text-xs text-muted-foreground font-sans tracking-widest uppercase whitespace-nowrap"
